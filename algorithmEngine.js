...
export const parseOdds = (oddsText) => {
...
export const impliedProbability = (oddsText) => {
...
export const identifyUndercoverWinner = (ranked) => {
  ...
  const scored = candidates.map((horse) => {
    const darkHorseScore =
      horse.valueEdge * 100 * 0.45 +
      numberOr(horse.lateKick) * 0.3 +
      numberOr(horse.improvingTrend) * 0.25;
  ...
  const templates = [
    {
      ...
      risk: "Low",
      stakeRatio: 0.3,
      ticket: `${top.name} to Win`
    },
    {
      type: "Place",
      risk: "Low",
      stakeRatio: 0.2,
      ticket: `${second.name} to Place`
    },
    {
      type: "Exacta Box",
      risk: "Mid",
      stakeRatio: 0.18,
      ticket: `${top.name} / ${second.name}`
    },
    {
      type: "Trifecta Key",
      risk: "Mid-High",
...
export const buildCounterBets = (ranked, bankroll, undercoverWinner) => {
  ...
  const valueAlternative =
    [...alternatives].sort((left, right) => right.valueEdge - left.valueEdge)[0] ??
    ranked[1] ??
    ranked[0];
  ...
  const templates = [
    {
      ...
      risk: "Mid",
      stakeRatio: 0.4,
      ticket: `${valueAlternative.name} to Win (against ${favorite.name})`
    },
    {
      type: "Exacta Fade",
      risk: "Mid",
      stakeRatio: 0.25,
      ticket: `${second.name} / ${third.name} (exclude ${favorite.name})`
    },
    {
      type: "Trifecta Fade",
      risk: "High",
      stakeRatio: 0.2,
      ticket: `${second.name}, ${third.name} over field (no ${favorite.name})`
    },
    {
      type: "Dark-Horse Saver",
      risk: "High",
      stakeRatio: 0.15,
      ticket: undercoverWinner
        ? `${undercoverWinner.name} to Place`
        : `${third.name} to Place`
    }
  ];
...
export const buildThreeTierSuggestions = (ranked, undercoverWinner) => {
  const sureCandidate =
    [...ranked]
      .slice(0, 3)
      .sort((left, right) => right.stability - left.stability)[0] ?? ranked[0];

  const midCandidate =
    [...ranked]
      .slice(0, 5)
      .sort((left, right) => right.valueEdge - left.valueEdge)[0] ?? ranked[1] ?? ranked[0];

  const longShotPool = ranked.filter((horse) => horse.marketProbability <= 0.12);
  const longShotCandidate =
    [...longShotPool].sort((left, right) => right.valueEdge - left.valueEdge)[0] ??
    undercoverWinner ??
  ...
  return [
    {
      tier: "Sure Bet",
      horse: sureCandidate,
      strategy: "Win or key in exacta/trifecta with top two model horses."
    },
    {
      tier: "Mid Bet",
      horse: midCandidate,
      strategy: "Win/place split with a smaller exacta box."
    },
    {
      tier: "Long-Shot Bet",
      horse: longShotCandidate,
      strategy: "Small win ticket plus saver place ticket."
    }
  ];
};
